function showAssignments(scheduleTable) {
    //TODO: make the assignments click listener change the localStorage (for both the main and missing assgaigments)
    ...
    for (let i = 0; i < classes.length; i++) {
        ...
        assignments.forEach((assignment) => {
            ...
            if (
                assignment.class === activityName &&
                assignment.due === scheduleDate
            ) {
                ...
                styling +=
                    "border-radius:6px;padding:5px;font-size:11px;margin:3px;";
                span.style = styling;
    ...
    OUTER: rows.forEach((r) => {
        ...
        INNER: for (let i = tds.length - 1; i >= 0; i--) {
            if (tds[i].getAttribute("data-heading") === "Activity") {
                ...
                link &&
                tds[i].getAttribute("data-heading") === "Block"
            ) {
                tds[i].innerHTML += "<br/>";
...
function loadSchedulePage() {
    ...
    let addListenersInterval = setInterval(
        () => addArrowListenersLoop(addListenersInterval),
        1000
    );
...
function addOfficeHoursLinks(loop) {
    ...
    if (mainColumn) {
        ...
        //adding list items
        let officeHoursLinks = JSON.parse(
...
function addClearButtonLoop(loop) {
    ...
    let table = document.querySelector(
        ".table.table-striped.table-mobile-stacked"
    );
...
function addArrowListenersLoop(loop) {
    ...
    if (leftArrow && rightArrow) {
        leftArrow.onclick = () => {
            document
                .querySelectorAll(".removeable-section")
...
function addMissingAssignments(schedule) {
    //TODO: make the click listener affect localStorage
    if (document.querySelector("#other-assignments-container")) {
        return;
    }
    let classes = [];
    let missingAssignments = JSON.parse(
        localStorage.getItem("assignment-data")
    );
    ...
    for (
        let i = 0;
        i < rows?.length;
        classes.push(rows[i++].children[2].innerText)
    ) {}
    //gets date
    ...
    missingAssignments = missingAssignments.filter((assignment) => {
        return (
            !classes.includes(assignment.class) &&
            assignment.due == scheduleDate
        );
    });
